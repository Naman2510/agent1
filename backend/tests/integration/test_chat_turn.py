...
from app.providers.llm.base import TokenUsage, ToolCall
...
async def test_turn_indices_increment(client: AsyncClient, registered, auth_headers) -> None:  # type: ignore[no-untyped-def]
    _, _, tokens = await registered()
    headers = auth_headers(tokens)
    session_id = await _session(client, headers)
...
async def test_the_prompt_prefix_is_byte_identical_across_turns(
    ...
    prefixes = [tuple(b.text for b in request.system if b.cacheable) for request in llm.requests]
    assert prefixes[0] == prefixes[1]


async def test_no_tools_are_offered_in_phase_2(
    client: AsyncClient, registered, auth_headers, llm: FakeLLMProvider
) -> None:  # type: ignore[no-untyped-def]
    _, _, tokens = await registered()
    headers = auth_headers(tokens)
