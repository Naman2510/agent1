...
// imem.sv
//
// Simulated instruction memory for Phase 2. Word-addressed, initialized
// from a $readmemh hex file (one 32-bit instruction word per line, as
// produced by scripts/asm_to_hex.py or by objcopy in later phases).
// Read is combinational: in a single-cycle CPU the fetched instruction
// must be available within the same cycle the new PC is presented, with
// no clock-edge latency, exactly like a fast SRAM/ROM being read
