#!/usr/bin/env python3
"""
collect_heldout_dataset.py -- Phase 14: run the 9 HELD-OUT scheduler
workloads (vecadd/dot N in {2,3,8,32}, matmul N=1 -- see
scheduler/benchmarks/gen_scheduler_programs.py's HELDOUT_* lists)
through sim/testbenches/tb_benchmark_soc.sv (the same generic,
sentinel-detecting harness Phase 11/13 use -- no changes needed, since
every held-out program here is cheaper than Phase 13's
cpu_matmul_n8), and write scheduler/training/heldout_dataset.csv.

This is intentionally a SEPARATE file from
scheduler/training/dataset.csv: dataset.csv is what
train_scheduler.py fits the model on, and heldout_dataset.csv is
GROUND TRUTH the model never saw during training, used only to score
its predictions in scheduler/inference/evaluate_accuracy.py. Mixing
the two files would make it impossible to tell a real held-out
evaluation from an inflated training-set accuracy number.

Correctness of every held-out program (the actual computed result,
not just its timing) is established by
sim/testbenches/tb_scheduler_heldout_correctness.sv
(`make test_scheduler_heldout_correctness`) -- run that first if you
have any doubt.
...
# Must match scheduler/benchmarks/gen_scheduler_programs.py's
# HELDOUT_VECADD_DOT_SIZES / HELDOUT_MATMUL_SIZES exactly.
PROGRAMS = []
for op, sizes in [("vecadd", [2, 3, 8, 32]), ("dot", [2, 3, 8, 32]), ("matmul", [1])]:
    for n in sizes:
...
def main():
    ...
    out_path = os.path.join(ROOT, "scheduler", "training", "heldout_dataset.csv")
